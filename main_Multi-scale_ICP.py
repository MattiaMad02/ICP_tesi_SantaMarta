...
def multiscale_icp(source, target, threshold, voxel_scales, max_iters):
    ...
    for i, voxel in enumerate(voxel_scales):
        ...
        last_rmse = reg.inlier_rmse
        print(f"       fitness={reg.fitness:.4f}, RMSE={reg.inlier_rmse:.4f}")
    return T, last_fitness, last_rmse
...
for i, source in enumerate(point_clouds):
    ...
    np.savetxt(
        os.path.join(save_folder, f"transformation_{i+1}.txt"), T
    )
    o3d.io.write_point_cloud(
        os.path.join(save_folder, f"aligned_{i+1}.ply"), source
    )
    summary_lines.append(
        f"Frame {i+1}: Fitness={fitness:.4f}, RMSE={rmse:.4f}\n"
    )
...
output_file = os.path.join(
    save_folder, "merged_multiscale_icp_map_robust.ply"
)
...
summary_file = os.path.join(
    save_folder, "multiscale_icp_robust_summary.txt"
)
...
screenshot_path = os.path.join(
    save_folder, "merged_multiscale_map.png"
)
